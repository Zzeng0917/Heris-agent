...
async def demo_full_agent():
    ...
    print("\n" + "=" * 60)
    print("Full Heris - All Features Enabled")
    print("=" * 60)

    # Load configuration
    config_path = Path("heris/config/config.yaml")
    if not config_path.exists():
        print("❌ config.yaml not found. Please run:")
        print("   cp heris/config/config-example.yaml heris/config/config.yaml")
        return
    ...
    with tempfile.TemporaryDirectory() as workspace_dir:
        ...
        system_prompt_path = Path("heris/config/system_prompt.md")
...
"""
        ...
        try:
            mcp_tools = await load_mcp_tools_async(config_path="heris/config/mcp.json")
            if mcp_tools:
...
async def demo_interactive_mode():
    ...
    print("\n" + "=" * 60)
    print("Interactive Heris")
    print("=" * 60)
    print("\nThis demo shows multi-turn conversation.")
    print("(In production, use `heris` for full interactive mode)")
