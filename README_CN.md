...
**Heris** 是一个极简但专业的演示项目，旨在展示使用 MiniMax M2.5 模型构建 Agent 的最佳实践。项目通过兼容 Anthropic 的 API，完全支持交错思维（interleaved thinking），从而解锁 M2 模型在处理长而复杂的任务时强大的推理能力。
...
此模式适合希望快速体验 Heris，而无需克隆代码仓库或修改代码的用户。

**安装步骤：**

```bash
# 1. 直接从 GitHub 安装
uv tool install git+https://github.com/MiniMax-AI/heris.git

# 2. 运行配置脚本（自动创建配置文件）
# macOS/Linux:
curl -fsSL https://raw.githubusercontent.com/MiniMax-AI/heris/main/scripts/setup-config.sh | bash

# Windows (PowerShell):
Invoke-WebRequest -Uri "https://raw.githubusercontent.com/MiniMax-AI/heris/main/scripts/setup-config.ps1" -OutFile "$env:TEMP\setup-config.ps1"
...
配置脚本会在 `~/.heris/config/` 目录下创建配置文件，请编辑该文件：

```bash
# 编辑配置文件
nano ~/.heris/config/config.yaml
```
...
```bash
heris                                    # 使用当前目录作为工作空间
heris --workspace /path/to/your/project  # 指定工作空间目录
heris --version                          # 查看版本信息

# 管理命令
uv tool upgrade heris                    # 升级到最新版本
uv tool uninstall heris                  # 卸载工具（如需要）
...
```bash
# 1. 克隆仓库
git clone https://github.com/MiniMax-AI/heris.git
cd heris
...
**Windows:**
```powershell
Copy-Item heris\config\config-example.yaml heris\config\config.yaml

# 6. 编辑配置文件
vim heris/config/config.yaml  # 或使用您偏好的编辑器
```
...
> 📖 完整的配置指南，请参阅 [config-example.yaml](heris/config/config-example.yaml)
...
Heris 支持 [Agent Communication Protocol (ACP)](https://github.com/modelcontextprotocol/protocol)，可与 Zed 等代码编辑器集成。

**在 Zed Editor 中设置：**

1. 以开发模式或工具模式安装 Heris
...
命令路径应为：
- 通过 `uv tool install` 安装：使用 `which heris-acp` 的输出结果
- 开发模式：`./heris/acp/server.py`

**使用方法：**
- 使用 `Ctrl+Shift+P` → "Agent: Toggle Panel" 打开 Zed 的 Agent 面板
- 从 Agent 下拉列表中选择 "heris"
- 直接在编辑器中开始与 Heris 对话
